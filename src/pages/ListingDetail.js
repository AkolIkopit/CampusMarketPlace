            ...
            <section className="seller-profile-card">
              <header className="seller-card-top">
                {listing.profiles.avatar_url ? (
                  <img src={listing.profiles.avatar_url} alt="" className="seller-img" />
                ) : (
                  ...
                  <article className="review-card">
                    <header className="reviewer-info">
                      {rev.reviewer.avatar_url ? (
                        <img src={rev.reviewer.avatar_url} alt="" className="reviewer-thumb" />
                      ) : (
                        <figure className="reviewer-placeholder"><User size={16} /></figure>
                      )}
                      <nav className="reviewer-name-group">
                        <strong>{rev.reviewer.full_name}</strong>
                        <time>{new Date(rev.created_at).toLocaleDateString()}</time>
                      </nav>
                      <figure className="review-stars">
                        {[...Array(5)].map((_, i) => (
                          <Star key={i} size={12} fill={i < rev.rating ? "#f3a91e" : "none"} color={i < rev.rating ? "#f3a91e" : "#ddd"} />
                        ))}
                      </figure>
                    </header>
